...
class MusicFragment : Fragment(R.layout.fragment_music) {
    ...
    private val binding by viewBinding<FragmentMusicBinding>()

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        binding.vpMusicVideo.apply {
            val musicVideoTitles = resources.getStringArray(R.array.music_video_title)
            adapter = MusicVideoPagerAdapter(this@MusicFragment)
            isUserInputEnabled = false
            TabLayoutMediator(
                binding.tbMusicVideo,
                this
            ) { tab, position ->
                tab.text = musicVideoTitles[position]
            }.attach()
        }
